...
const checkIfUserLoginExists = (userDB, userReq) => userDB === userReq;

const checkIfUserIdExists = (userDB, userReq) => userDB === userReq;

const checkIfUserPasswordMatches = (passwordDB, passwordReq) => passwordDB === passwordReq;
...
router.post('/create_user', validator.body(createBodySchema), (req, res) => {
  ...
  users.forEach((currUser) => {
    const isUserAlreadyExists = checkIfUserLoginExists(currUser.login, req.body.userName);
...
router.post('/delete_user', validator.body(deleteBodySchema), (req, res) => {
  ...
  if (user) {
    const isPasswordMatches = checkIfUserPasswordMatches(user.password, req.body.userPassword);
...
router.post('/find_users', validator.body(findBodySchema), (req, res) => {
  const users = db.get('users');
  const usersFiltered = getAutoSuggestUsers(users, req.body.userName, req.body.userLimit);
...
router.post('/get_user', validator.body(getBodySchema), (req, res) => {
  ...
  users.forEach((currUser) => {
    const isUserAlreadyExists = checkIfUserIdExists(currUser.id, req.body.userId);
    if (isUserAlreadyExists) {
      user = currUser;
    }
  });
